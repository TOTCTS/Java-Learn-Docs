...
### 2. [后端框架](./spring/introduction.md) (待撰写)
**定位**：应用开发与业务实现。
...
### 3. [数据存储](./database/introduction.md) (待撰写)
**定位**：持久化、缓存与检索方案。
...
### 4. [消息队列 (MQ)](./mq/introduction.md) (待撰写)
**定位**：异步解耦与削峰填谷、事件驱动架构。
**核心内容**：Kafka（主题/分区/副本、生产/消费、事务与幂等）、RabbitMQ（交换机/队列/绑定、Confirm/DLX）、一致性与重试补偿、Schema 治理与观测。
本模块聚焦系统的异步化与鲁棒性建设，围绕交付语义、顺序性、背压与补偿机制，构建稳定可观测的事件流平台。

### 5. [微服务生态](./microservices/introduction.md) (待撰写)
**定位**：分布式系统架构与治理。
...
### 6. [工程化与 DevOps](./devops/introduction.md) (待撰写)
**定位**：软件交付与运维自动化。
